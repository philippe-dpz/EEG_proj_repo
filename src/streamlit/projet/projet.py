...
st.markdown(
    "Dans le cadre de notre formation en data science nous avons choisi d'étudier l'analyse d'enregistrements d'électroencéphalogrammes (EEG) et plus précisément l'imagerie motrice (IM)."
)
st.markdown(
    "L'IM correspond au schéma enregistré par l'électroencéphalogramme lorsque le sujet imagine l'exécution d'un mouvement sans réaliser celui-ci."
)
st.markdown(
...
st.markdown(
    "Il s'agit donc de classifier selon 2 classes, main gauche (classe 1) et de la main droite (classe 2), les enregistrements des différentes tentatives."
)

st.markdown("## Protocole d'acquisition des données")
st.markdown(
    "L'activité électrique du cerveau est enregistrée grâce à des capteurs placés sur le cuir chevelu."
)
st.markdown("Nous avons utilisé la base de données ‘BCI Competition IV Dataset b’")
st.markdown(
    "Pour recueillir les données 9 sujets ont participé à 5 sessions d'enregistrements EEG comprenant différentes conditions expérimentales."
)
st.markdown(
    "Les deux premières contiennent des données d'entraînement sans retour d'information (screening), et les trois dernières ont été enregistrées avec retour d'information (neurofeedback(nf))."
)
...
st.markdown(
    "Le détail du déroulement des sessions d'enregistrement se trouve  "
    """<a href="https://www.bbci.de/competition/iv/desc_2b.pdf">ici</a>"""
    "",
    unsafe_allow_html=True,
)
st.markdown(
    "Vous pouvez télécharger le "
    """<a href="app/static/rapport.pdf">rapport</a>"""
    " complet de notre étude.",
    unsafe_allow_html=True,
)
